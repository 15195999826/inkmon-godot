...
##
## 期望 damage 总数 = 2 (Strike x2) + 3 (SwiftStrike 三连击 HIT1/HIT2/HIT3) = 5
extends Node
...
func _on_battle_finished(timeline: Dictionary) -> void:
	...
	if dmg_count != 5:
		_fail("expected 5 damage events (Strike x2 + SwiftStrike x3 hits), got %d" % dmg_count)
		return
	...
	_pass("damage=5; granted configs=%s" % str(grant_configs.keys()))
