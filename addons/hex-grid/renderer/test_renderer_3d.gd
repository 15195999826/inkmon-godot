...
## 3D 六边形网格渲染器测试场景
##
## 用于手动验证 HexGridRenderer3D 的功能。
## 提供简单的 UI 按钮来测试渲染、高亮、填充和清除功能。
##
## 场景结构要求：
## - HexGridRenderer3D 节点（命名为 HexGridRenderer3D）
## - Camera3D 节点（俯视角度，便于观察网格）
## - DirectionalLight3D 节点（照明）
## - UI 按钮（连接到对应的回调函数）

@onready var renderer: HexGridRenderer3D = $HexGridRenderer3D
