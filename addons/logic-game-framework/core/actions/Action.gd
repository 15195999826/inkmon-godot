...
class BaseAction:
	...
	func _init(params: Dictionary):
		...
		elif selector_input is Callable:
			# 兼容旧代码：从 Callable 创建选择器
			_target_selector = TargetSelector.from_callable(selector_input)
		else:
			# 默认使用 CurrentTarget 选择器
		...
		for event in result.events:
			...
			for callback in triggered:
				...
				if action == null or not action.has_method("execute"):
					continue
				var callback_result = action.execute(callback_ctx)
