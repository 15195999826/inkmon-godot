...
## A* 寻路结果
class PathResult:
	var path: Array = []  ## 路径 (HexCoord 数组，包含起点和终点)
	var cost: float = 0.0           ## 总代价
	var found: bool = false         ## 是否找到路径
	
	func _init(p_path: Array = [], p_cost: float = 0.0, p_found: bool = false) -> void:
		path = p_path
		cost = p_cost
		found = p_found
...
static func astar(
	...
	while not frontier.is_empty():
		var current = _heap_pop(frontier)[1]  # HexCoord
...
static func astar_simple(
	...
	var is_passable := func(coord) -> bool:  # HexCoord
		return model.is_passable(coord)
	
	var cost_func := func(from, to) -> float:  # HexCoord
...
static func reachable_with_cost(
	...
	while not frontier.is_empty():
		var current = frontier.pop_front()  # HexCoord
		var current_key: String = current.to_key()
...
##
## @param model: 网格地图模型
## @param origin: 观察点 (HexCoord)
## @param target: 目标点 (HexCoord)
...
static func is_visible_simple(
	...
	var blocks_vision := func(coord) -> bool:  # HexCoord
...
static func raycast_to_simple(
	...
	var blocks_ray := func(coord) -> bool:  # HexCoord
		return model.is_tile_blocking(coord)
...
static func flood_fill_simple(
	...
	var is_same_region := func(coord) -> bool:  # HexCoord
