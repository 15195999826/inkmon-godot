...
### M2.1 — Economy (Worker Harvest, gold + wood) 🚧 active
...
**关键交付**:
- `RtsResourceNode` actor + `UnitClass.WORKER` + harvest activity / drop-off
- 双资源 (gold + wood) cost 全链路迁移
- crystal_tower 兼 drop-off (复用既有建筑, 不加 town_hall)
- 闭环: worker harvest → 资源到达 cost → 玩家 / AI enqueue PlaceBuildingCommand → 自动放兵营
