...
- [x] M5.1 — Data classes(2026-05-04)— RtsLongPathRequest / RtsWaypointPath / RtsPathGoal
- [x] M5.2 — RtsLongPathfinder A*(2026-05-04)— 5 元组 lex + 整数 cost + reverse-stored + direct-path fallback
- [x] M5.3 — Facade 雏形(2026-05-04)— compute_path_immediate / direct + is/make_goal_reachable;hierarchical make_goal_reachable_pathgoal
- [x] M5.4 — Activity / command 迁移(2026-05-04)— nav_agent facade 优先 + canonicalize 参数 + activity 端按语义区分
- [~] M5.5 — RtsBattleGrid facade 移除:**M5.5a navcell_grid 一等公民**(2026-05-04)done;**M5.5b-e RtsBattleGrid 完整删除 DEFERRED 到 EPIC 末 cleanup phase**(用户决策推迟 8-10h wallclock 纯 cleanup work,production code 已走 NavcellGrid 直接,删除不影响 functionality)
- [x] M5.6 — Smokes + Validation(2026-05-04)— 3 long_pathfinder smoke + 新 baseline CSV 968343 bytes + 全套 PASS
- [x] M5 整 milestone done + archived(2026-05-04)— `archive/2026-05-04-rts-m3-m5-long-pathfinder/`
