# Current State — 2026-05-04 baseline (M3 Epic / M0+M1+M2+M3+M4 done; M5 待启动)

inkmon-godot baseline 事实快照. M3 Epic / M5 启动用.

> **Active feature**: ⏸ 等用户审 M4 archive + 授权启 M5(LongPathfinder).
>
> **M0 + M1 + M2 + M3 + M4 sub-feature**: ✅ 整体完成 + archived
...
>
> **M3 Epic 状态**: codex Round 1-8 APPROVE + M0+M1+M2+M3+M4 done + 8 RTS smoke + 4 hierarchical smoke + LGF 73/73 + replay seed=42 deep-equal + baseline CSV byte-identical(829520 bytes,M3 inflate 接受新 baseline,M4 不消费)+ `smoke_clearance_inflate` 4 sub-test 全过 + `smoke_hierarchical_*` 4 个全过.
>
> **M4 deferred → M5**:✋2 体验点 + M4b.3 wire 进 RtsMoveUnitsCommand(语义冲突,等 M5 LongPathfinder 落地后 wire);M4c dirty 增量 → CANCEL(perf-gate realistic demo p99=28 ms ≤ 阈值,未来 grid 扩大可补).
>
...
- **M4**(HierarchicalPathfinder full recompute + canonicalize API):
  - `RtsRegionIdHelper` packed int64 (24+24+16 bit) — pack/unpack helpers,`is_invalid` 仅检 r 字段
  - `RtsHierarchicalChunk` 96×96 navcells per chunk + `regions: PackedInt32Array`(0=impassable,≥1=local region ID)+ `regions_id: Array[int]` 列表
  - `RtsHierarchicalPathfinder` per-class chunks + edges Dictionary[packed RID → Array[packed RID]] + global_regions Dictionary[packed RID → GlobalRegionID(单调递增)]
  - `recompute(grid, classes)` per-class flood-fill BFS(cursor + PackedInt32Array O(N) 起点 (lj, li) 字典序)+ `_build_edges` 跨 chunk 双向 edges(bsearch+insert 升序)+ `_compute_global_regions` 全量起点 BFS 含 isolated region(R5 P1 #3 修订)
  - 公开查询 API:`is_recomputed()` / `get_region(i,j,mask)` / `get_global_region(i,j,mask)` / `is_goal_reachable_point(start,goal,mask)` / `make_goal_reachable_point(start,goal,mask)` / `find_nearest_passable_navcell(start,mask)` + 内部 spiral ring scan
  - **M4b 阶段语义偏离 spec**:`make_goal_reachable_point` reachable → no-op(保 baseline 不漂),不可达 → 跟 start 同 GlobalRegion 离 goal 最近 navcell 中心。原 spec "总是 navcell 中心 mutate" 推迟 M5 LongPathfinder 落地 + 接受 P1 baseline 漂
  - Wire — `world.hierarchical_pathfinder` 字段 + `procedure._init` 末构造 + `tick_once` step 6.7 lazy recompute(`is_recomputed()` derived state 守卫)
  - Production code 不消费 M4 API → 0 baseline 漂移(M4b.3 wire DEFERRED 到 M5)
  - **Perf**:realistic demo(96² + 16 building × 5²)recompute p99=28 ms / 30 ms 阈值;synthetic future-warning(192²/384²/768² + 10% scattered)p99=119/450/1683 ms 仅 info 不阻塞;每 navcell ~3 us GDScript BFS overhead
...
## 测试基线 (M4 末态 = 17 项 + LGF 73 + 3 obstruction_manager + 1 clearance_inflate + 5 hierarchical smoke)
...
| `tests/battle/smoke_hierarchical_perf.tscn` | (M4 新)realistic demo p99=28 ms ≤ 阈值 PASS;synthetic future-warning info |
...
- 主仓 master ahead of origin/master(M3 Epic + M0-M4 实施期间累积 commit 待推)
- submodule `addons/logic-game-framework` HEAD=5bd9a36(M4 末态:M4a + M4b + M4-perf-gate)
- M4 archive sweep 通过 0677a43 + 后续 archive commit 完成

## 决策来源 (历史 sub-feature → archive)

- **M4 HierarchicalPathfinder** (2026-05-04): `archive/2026-05-04-rts-m3-m4-hierarchical/` ← **最近**
