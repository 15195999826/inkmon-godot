# Current State — 2026-05-04 baseline (M3 Epic / M0 + M1 done; M2 active)

inkmon-godot baseline 事实快照. M3 Epic / M2 启动用.

> **Active feature**: M3 Epic / M2 (ObstructionManager Shape 数据库).
>
> **M0 + M1 sub-feature**: ✅ 整体完成 + archived
...
>
> **M3 Epic 状态**: codex Round 1-8 APPROVE + M0 + M1 done + 14 项 smoke 0 漂移 + replay deep-equal + baseline CSV byte-identical.
>
...
## 当前 baseline 能力 (M1 末态 = M2 出发点)

M2 末态 RTS 完整可玩 1v1 skirmish + AI vs AI 观战 demo 之上,M0 + M1 增量:
...
- `RtsBattleGrid` 改 facade:`_navcell_grid` + `_passability_registry` + `_default_class_mask` + `_half_cols`/`_half_rows` 字段;`attach_passability_registry` (lazy 创建 NavcellGrid + 从 model.get_all_coords sync 已有 blocking) + `is_blocking` + `mark_obstacle_cell` + `_coord_to_ij` helper;`is_passable_for_layer` 走 `is_blocking`;`place_building` / `remove_building` 双写
...
- 期间 hotfix(M0 阶段 + M1 期间均无新增 hotfix,M0 hotfix 见 M0 archive Summary)
...
- **Trace 基础设施** (M0.1):
  ...
  - `addons/.../tests/baselines/0ad-baseline-master.csv` (882 KB,byte-identical 跨 run + M1 末态 byte-identical)
...
| `tests/battle/smoke_navcell_grid_passability.tscn` | (M1 新)AC1+AC2+AC8 13 项断言全过 |
...
- 主仓 master ahead of origin/master(M3 Epic + M0 + M1 实施期间累积 commit 待推)
- submodule `addons/logic-game-framework` HEAD=d077c54(M1 末态)
- M1 archive sweep 待 commit(本文件 + Progress / Next-Steps / m3 README / M1.md status 修订)

## 决策来源 (历史 sub-feature → archive)

- **M1 Navcell Grid + Passability** (2026-05-04): `archive/2026-05-04-rts-m3-m1-navcell-grid/` ← **最近**
