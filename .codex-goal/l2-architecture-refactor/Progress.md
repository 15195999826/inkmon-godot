...
- P7 UI .tscn ✅ — HUD/drawer/modal = `ui/{hud_content,right_drawer,save_load_modal}.tscn`; 动态列表 = `ui/components/{roster_chip,party_entry_row,bag_item_row,npc_action_row}.tscn` instantiate; director 只取引用+绑 data+连 signal+驱动 transition。
- P8 装备+刻印+存档 ✅(1 项 resolved,见下) — 装备 stat 折叠 `ink_mon_game_session.gd` _fold_equipment_stats; 刻印 LGF 被动 `ink_mon_engraving_passive.gd`(projection→actor→grant); 多槽 save_to_slot/load_from_slot/list_save_slots + `save_load_modal.tscn` 槽行。
...
**Non-Goals (均未违反):** 不改 addons(`git diff 04380e5...HEAD -- addons/` 空)✅; 不动 hex-atb-battle(空)✅; lab 数据=手搓 stub ✅; 无技能数值变异(roll 只选 skill_id)✅; 无 PvP/网络/IV ✅; 不调平衡(level-1 derive==config base,smoke 断言)✅; from_dict 无迁移(version 检查+丢弃)✅。
...
Consistency review: 1 items resolved (P8 Phase-G 误归属 → 项目本地等价实现); 其余 §1-§8 逐条一致, 无未解决 divergence。
