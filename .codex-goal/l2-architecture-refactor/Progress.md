...
- P7 UI .tscn 🔧(主体达成; 残留少量自绘 = F3) — HUD/drawer/modal = `ui/{hud_content,right_drawer,save_load_modal}.tscn`; 动态列表 = `ui/components/{roster_chip,party_entry_row,bag_item_row,npc_action_row}.tscn` instantiate; director 取引用+绑 data+连 signal+驱动 transition。残留 4 CanvasLayer(分层) + prompt button(跑位) + 个别 fallback Label/StyleBox 未尽 arch §96「零自绘」→ 见 Known Divergences F3。
...
**Known Divergences (2, accepted for v1 — 无 pending fix):**
...
- **F3 — P7 薄场景残留运行时自绘 (accepted / 完成度偏差)** — 要求:docs/L2-ARCHITECTURE.md:88/96「全 .tscn / 零 UI 自绘」。现状:HUD/drawer/modal/动态列表(主体 ~85%)已抽 .tscn instantiate ✅;`ink_mon_game_director.gd` 仍剩 4×`CanvasLayer.new()`(分层基建,:676-707)+ prompt `Button.new()`(随 NPC 跑位)+ 个别 fallback `Label.new()`/system button/per-chip `StyleBoxFlat`(空态/边缘)。处置:**用户拍板接受** —— 残留多为合理 tactical 例外(CanvasLayer 非设计态 UI;prompt button 要运行时定位),非 §90「100% Button.new()」反模式,不为纯洁性重构。复核结论:**完成度偏差,非阻塞**。

Consistency review: 2 resolved (P8 Phase-G 误归属 → 项目本地等价实现; F2 旧存档丢弃重开已修+测); 2 accepted divergences (F1 刻印 target_slot v1 占位; F3 P7 残留少量自绘)。**无 pending divergence** —— F1/F3 经用户拍板接受为 v1 scope。Gate:inkmon 5 组 7/7 + -Required 9/9 全退 0(F2 fix 后复跑 PASS)。
