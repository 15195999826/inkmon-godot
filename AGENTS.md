...
| **单技能契约** `tests/smoke_strike.tscn` | 具体 skill 的数值/tag/effect 断言 |
| **全链路冒烟** `tests/smoke_frontend_main.tscn` | main.tscn → core 战斗 → replay → director 播放（~80% 回归面） |
...
- **改 LGF core / 框架层**：跑单元测试 + `smoke_frontend_main`；新机制加 `*_test.gd` 登记到 `addons/logic-game-framework/tests/run_tests.gd::TEST_PATHS`
- **开发新技能**：写 `tests/smoke_<skill>.tscn`（模板：`smoke_strike.gd/.tscn`）断言数值和 tag；再跑 `smoke_frontend_main` 确保集成不崩
- **修 bug**：找覆盖到该路径的 smoke 复现，修完跑同一个验证

### 写 smoke scene 的约定

模板 `tests/smoke_strike.gd`：
...
项目自带两个 Codex Skill（`.Codex/skills/`，按需加载，不常驻 context）：
...
配套 slash command（`.Codex/commands/`）：
