...
func get_interpolated_hex(progress: float) -> Vector2:
	var eased_progress := InkMonRender2DVisualAction.apply_easing(progress, easing)
	return Vector2(
		InkMonRender2DVisualAction.lerp_value(float(from_hex.q), float(to_hex.q), eased_progress),
		InkMonRender2DVisualAction.lerp_value(float(from_hex.r), float(to_hex.r), eased_progress)
	)
