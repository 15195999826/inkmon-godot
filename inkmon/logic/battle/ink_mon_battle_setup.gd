...
static func available_coords(gi: InkMonWorldGI) -> Array[HexCoord]:
	...
	for coord in board.get_all_coords():
		if board.is_passable(coord) and not board.is_reserved(coord):
			result.append(coord)
...
static func pop_first_available(gi: InkMonWorldGI, coords: Array[HexCoord]) -> HexCoord:
	...
	while not coords.is_empty():
		...
		if board.has_tile(coord) and board.is_passable(coord):
			return coord
	return null
...
## configure_grid(_model) 是 stdlib GridWorldGameplayInstance 的棋盘钩子 (落到 gi.grid = 战斗棋盘), 此处委派过去。
