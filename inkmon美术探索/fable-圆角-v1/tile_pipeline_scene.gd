...
func _capture_if_requested() -> void:
	...
	if viewport_texture == null:
		push_error("tile_pipeline: capture failed %s viewport texture is null" % target_path)
		get_tree().quit(1)
		return
	var image := viewport_texture.get_image()
	if image == null:
		push_error("tile_pipeline: capture failed %s viewport image is null" % target_path)
		get_tree().quit(1)
		return
	var err := image.save_png(target_path)
	if err != OK:
		push_error("tile_pipeline: capture failed %s err=%d" % [target_path, err])
		get_tree().quit(1)
		return
	get_tree().quit(0)
