...
# 聚焦某 unit / tick 段 / 某事件
python .Codex/skills/sim-nav-map-bugfix/probe_log.py <log.json> --unit blue_2 --tick-range 9700:9900
python .Codex/skills/sim-nav-map-bugfix/probe_log.py <log.json> --kind movement_line_blocked
```
...
```bash
# 1. cp 模板到 .Codex/tmp/
cp .Codex/skills/sim-nav-map-bugfix/probe_los.{gd,tscn} .Codex/tmp/
# 2. Edit start/target/blockers 为最小复现条件
# 3. 跑（30s 内出结果）
godot --headless --path . .Codex/tmp/probe_los.tscn 2>&1 | grep "LOS\|FACADE\|LAB_PF"
```
