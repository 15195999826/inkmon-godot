...
description: 即将推荐架构模式、重构、拆分,或在设计 trade-off 上犹豫时调用 —— 即使用户没主动问。提供 19 章 Game Programming Patterns 简述 + 项目已落地机制对照,先看已有再推荐,阻止凭训练记忆即兴答。
---
...
**默认不查原文**,简述够用。仅在下列情况点开简述末尾的外链 `https://gpp.tkchu.me/<name>.html`(在 Codex 中用可用的网页浏览/抓取工具读取):
