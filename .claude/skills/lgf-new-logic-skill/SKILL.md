...
│       ├── core/                          (框架)
│       ├── stdlib/                        (复用 Action/Component/Systems/Replay)
│       └── example/
│           ├── hex-atb-battle/core/       核心层:WorldGI / Procedure / events (跨战斗持久化)
│           ├── hex-atb-battle/            ← 逻辑层,技能实现写在这里 ★
│           │   ├── skills/                (主动技能 AbilityConfig)
│           │   ├── buffs/                 (buff 定义,如 HexBattlePoisonBuff)
│           │   ├── actions/               (伤害/治疗/反伤/自定义 Action)
│           │   ├── target_selectors.gd    (TargetSelector 形状注册)
│           │   ├── character_actor.gd     (Actor 子类)
│           │   └── ...
│           └── hex-atb-battle/frontend/   表演层:Node3D / 动画 / UI (只订阅 event)
└── tests/skill_scenarios/                 ← scenario 测试(外层) ★
```

**三层分工**(详见 `addons/logic-game-framework/example/hex-atb-battle/core/README.md`):
- **hex-atb-battle/core** 持久、跨战斗、Godot-free 的 world + procedure + 事件类型。
- **hex-atb-battle** 战斗逻辑(本 skill 99% 的写作目标):技能/Buff/Action/Actor 子类/AI/TargetSelector。
- **hex-atb-battle/frontend** 视觉层,本 skill 按 §7 留占位给"回家后"接入。

**90% 的新增技能代码都在 `hex-atb-battle/` 这一层**;core 和 frontend 只在极少数下沉原语或绑定 cue_id 时才碰。
...
3. **完整代码骨架**:`.gd` 文件级别的 ABILITY / Action / Resolver / Buff 草案 — 含**所有** const(CONFIG_ID / TIMELINE_ID / COOLDOWN_MS / 阈值 / 数值)、`ability_tags`、`meta()` key、StageCue cue_id(注明复用哪个 vs 新增,见 §7.3)
4. **Timeline 节奏**:具体 timeline_id + duration + 每个 tag 的 time_ms
