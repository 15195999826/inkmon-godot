...
Condition 是**事件到达时的 reactive 判断**, 适合"该不该响应这个事件"——无论 ability 本身是被动 (buff/反伤) 还是主动技能的门控 (如 `HexBattleCooldownSystem.apply_standard_active_gating` 里的 cooldown/行动状态检查) 都算它的本职; 它不是主动 cast 时的目标合法性判断 (range / faction / target kind 这类 cast 前过滤).
...
# 在 can_use_skill_on
var allowed: Array = skill.metadata.get(HexBattleSkillMetaKeys.ALLOWED_TARGET_KINDS, [HexBattleActor.KIND_CHARACTER])
if not target.type in allowed:
    return false

# 在技能配置 (AbilityConfig.builder() 链, 例见 wall_breaker.gd)
.meta(HexBattleSkillMetaKeys.ALLOWED_TARGET_KINDS, [HexBattleActor.KIND_CHARACTER, HexBattleActor.KIND_ENVIRONMENT])
```

AI / UI / tooltip 都直接读 `skill.metadata.get(...)`, 不需要任何 dry-run. (`metadata` 是 `Ability`/`AbilityConfig` 上的 plain `Dictionary` 字段, 不存在通用的 `get_meta()` 方法 —— 只有 `get_meta_int()` 这个整数专用取值 helper.)
...
4. 默认值放在第 2 步的 `metadata.get(KEY, DEFAULT)` 里, 这样**现有技能零修改**
