...
- **Self-activation is declared, not passed**: `grant_ability(ability)` always delivers `AbilityGranted` to the newly granted ability itself (directed, via `deliver_to_ability`; siblings and other actors don't hear it); whether an ability self-activates is decided by its own trigger (`TriggerConfig.GRANTED_SELF`).
...
- **Post events are subscriptions**: `process_post_event(event_dict)` takes no audience. An ability subscribes in `apply_effects` for every kind its components' triggers name (`get_post_event_kinds()`) and unsubscribes in `remove_effects`; a component that overrides `on_event` without declaring kinds only gets directed deliveries. Whether a dead / stunned owner still reacts is the actor's `is_event_responsive(event_dict, phase)` (pre and post), never a list the caller builds. Directed events — the procedure's activation request, the `AbilityGranted` notice `grant_ability` sends to the new instance, a projectile outcome sent back to its launcher — go through `instance.event_processor.deliver_to_ability(event, owner_id, ability_id)` and reach only that ability's `.direct()` triggers (`TriggerConfig.ABILITY_ACTIVATE` / `GRANTED_SELF` are direct, with no id precheck); `AbilitySet` has no event entry point, and both processor routes converge on `Ability.receive_event`.
- **Time-driven components hand over a function, not a type and not a flag**: `AbilityComponent` has no tick hook called by name. A component whose state moves with time overrides `get_tick_callable()` and returns its own `(dt: float) -> void` function (which guards `is_active` itself, as `TimeDurationComponent` does); `Ability` collects them at construction (`needs_tick()`), and `AbilitySet.tick` skips the ability traversal when no ability handed one over, judged afresh every frame from the live list — no counter, no flag. `tick_executions` likewise returns early when nothing is executing. Writing a method named `on_tick` does nothing.
- **Expiry is reclaimed by the path that ran it**: an ability that `expire()`s itself while being run — in `tick` / `tick_executions` (`_process_abilities`), inside its own post handler, or inside a directed delivery (`deliver_to_ability`) — is revoked by that same path in the same pass. Call `revoke_ability` yourself only for external removal (cleanse, unequip, replace, a shield broken by damage); never on death. `expire` = the ability ends itself and drops its effects, `revoke` = the set strikes it off and notifies; two steps by design (no back-reference), not one.
- **Event dict keys and kind literals are snake_case**: every key an event `to_dict()` / factory emits (`GameEvent`, `ProjectileEvents`, `PlaybackData`, the `RawAttributeSet` listener dict, ability / component `serialize()`) and every `kind` string matches `^[a-z0-9_]+$` — read them through the constants and `from_dict()`, never a camelCase spelling; action / condition / cost type ids (`type` / `get_condition_type()`, e.g. `loose_tag_apply`, `has_tag`, `consume_tag`) follow the same spelling; `tests/core/events/event_key_casing_test.gd` gates both.
...
- [ ] Post events go through `process_post_event(event_dict)` with no audience list; death policy lives in the actor's `is_event_responsive`; activation requests go through `instance.event_processor.deliver_to_ability(event, owner_id, ability_id)`
