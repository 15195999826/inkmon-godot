...
python .claude/skills/sim-nav-map-bugfix/probe_log.py <log.json> --unit fast_0
python .claude/skills/sim-nav-map-bugfix/probe_log.py <log.json> --kind order_failed --tick-range 200:600
```
...
- 临时 probe 放 `.claude/tmp/`（gitignored），`.tscn` 三行指向脚本，`godot --headless --path . .claude/tmp/<probe>.tscn`
