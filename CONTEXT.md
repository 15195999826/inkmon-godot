...
inkmon-lab（MCP 操作活的 lab app，要求 lab 常驻打开），Blender/Godot 环节在本仓
`inkmon/tools/tile_pipeline/`。
...
- **UV 展开版** — 顶面 island + 可见侧壁矩形的展开布局；`design_uv_gpt` 第二步用
...
AI 调 `generate` 主用 session 模式（继承配方 + 追加描述 + 自带几何底图），裸模式留实验兜底。
...
+ `blender/textures/`（UV 贴图 + 每资产 provenance JSON：prompt/方案/参考图/lab session id）。
...
- **设计稿** 按所选**生图方案**派生 **UV 贴图**；相机角度已冻结为契约（adr/0009）
...
> **Dev:** "这张草地**设计稿**用户批了，直接当贴图用？"
> **Domain expert:** "不行——设计稿是 3D 视角的压扁图，先按 manifest 相机角度 warp 回
> 正六边形得到 **UV 贴图**，贴进 Blender 材质重烘，**烘焙 PNG** 才是 Godot 用的东西。"
