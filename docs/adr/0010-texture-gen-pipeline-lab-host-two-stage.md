...
3. **车间/入库两段制**。lab = 车间：全部生成历史含裸模式（~~自动 scratch session，无例外~~
   **2026-06-12 修订**：scratch session 方案废弃——session 专属用户沉淀的配方模板，裸模式不再
   制造 workspace/session 容器，改落**独立调用记录**（lab `texture-gen/calls/<callId>/`，含
   提示词/输入图副本/模型参数/耗时/产出），lab header ··· 菜单「裸模式生图」tab 纯结果展示；
   image id 跨来源稳定，history/export/tag 不受影响；存量 scratch 已迁移，Round 2.5 及更早
   provenance 里的 `lab_session_id` 为历史指针，`lab_image_id` 仍可解析）；
   AI 永不直接收图字节，拿文件唯一通道是 export。godot 仓 = 入库真相：批准品进
   `blender/designs|textures/`（含 provenance JSON：prompt/方案/参考图/lab session+image id）并
