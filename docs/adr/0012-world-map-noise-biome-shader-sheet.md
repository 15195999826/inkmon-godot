...
单独成类（`InkMonMapBakeMath`）。渲染差异定位的固定手段 = **mock_replica.tscn**（Godot 内
逐像素还原用户拍板基准图 mock_style_C, 均差 1.8% 噪声级；任何渲染质疑先跑它裁决）。
...
## 2026-07-05 A1+A2+A3 修订（用户拍板"按 A1+A2+A3 执行"）

复刻实验（mock_replica 逐像素还原基准图, 均差 1.8%）证明渲染技术无损后, 残余观感差定位为
展示几何与噪声风味, 三项一并落地:

- **A1 大地图去等轴压扁**: `ISO_SQUISH` 0.5→1.0。大地图语义 = 摊开的"世界地图纸", 不迁就
  据点/战斗的 2:1 等轴——压扁把山形/海湾压成横椭圆, 是"游戏内不如基准图"的最大单一因素
  (证据 squish_evidence.png)。节点/走廊/迷雾/点击随几何自动跟随。
- **A2 生成噪声换 mock 值噪声**: FastNoiseLite → uint32 整数 hash 值噪声 (与基准图同族地貌;
  跨平台位级确定, 优于 FastNoiseLite 的入档语义)。场/海岸/温度公式与 mockgen 逐字同值,
  常量收敛进 `InkMonWorldMapData` (与 GPU 场 shader 成对)。陆地矩形取 mock 几何 (宽+0.5,
  margin 6.5)。`SAVE_VERSION` 5→6。
- **A3 渲染切 GPU 多 pass**: InkMonMapBakeMath 重构为 GPU 管线 (fields→CDF→rank→box×3
  模糊链×4→pack, 40px/单位 = 基准图同密度, ~1s/世界, 异步烘焙热替换; headless 走 CPU 低密度
  fallback 保链路)。风格 shader 改吃预烘场 + mock 十类分类 (snow/rock/mount/hills/tundra/
  beach/dense/forest/desert/dry/grass, 各风格色板补 rock/dense/desert), 合成顺序与 mock 逐字
  对齐 (海带→biome→hillshade→描线→斑驳→post_darken→纸纹→暗角)。面片/墨线风格化保留。
