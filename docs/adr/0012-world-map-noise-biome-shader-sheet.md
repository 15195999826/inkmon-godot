...
外加**分位 CDF LUT**（raw→秩01，biome 阈值在秩域=覆盖率语义，与逻辑层分类同域同阈值）；
上色/分类/海岸/浅滩/hillshade/墨线/纸纹全部 shader 逐像素完成（任意缩放不糊、Web 导出兼容、
调参即时）。入档 cells 仍是玩法/环境唯一真相。**对齐口径 = 同源近似而非 bit 级**：逻辑侧的
