...
| 🟡 ✅ (R2) | damage post-event 用 pre-damage alive list，死者仍收 post-damage/death 事件（未来 lifesteal/thorns/death-reaction 会污染）。**2026-09-12 LGF core 重构 P5 已解决**：post 事件改订阅派发、不再传观众名单，死者响不响应由 actor 的 `is_event_responsive` 决定（inkmon 用 `BattleActor` 默认 `not is_dead()`） | `inkmon/logic/battle/actions/ink_mon_damage_action.gd` | 不再适用（机制已换） | S |
