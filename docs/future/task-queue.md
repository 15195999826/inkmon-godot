...
### 1d. 寻路/移动 scaling 储备刀（触发式，2026-07-02 登记）
性能现状：跨图规划 0.8ms/条（每 tick ≤1 条恒定，不随单位数涨）；建表已预热进 rebuild（首查不再付 ~10ms）；8 单位移动稳态 tick ~0.78ms、静止 ~0.04ms。按触发条件动刀，**不提前做**：
- **技能造墙（牛头人/沟壑类，用户 2026-07-02 提出的需求）**：战斗内动态加/删障碍。现有链路已正确（obstruction 变更 → dirty → 表自动失效重建 ~10ms/次，A/B+smoke 锚定），施放低频时 v1 直接可用。**升级刀 = ray 表增量修复**：脏格只作废穿过它的行/列射线段，修复 O(受影响段长) 亚毫秒级，替代全图重建。触发：1c 技能系统落地、造墙技能进战斗循环时。
- **LOS 抄近检查查表化**（`is_line_walkable`，每移动单位每 tick ≤2 次）：触发 = 单位规模 30+。⚠️ 语义雷区已勘探：`validate_movement_line` = 栅格走线（带 0 A.D. 逃逸规则：可从不可通行格走出）+ 第二段精确几何形状检查，**不是**机械换 `segment_clear`；需带逃逸规则的 baked 孪生 + 保留形状段 + 独立 A/B。预期 8 移动单位 tick 0.78→~0.4ms，收益随单位数线性。
- **分离求解空间哈希**：O(N²) 是单位数真天花板（9 单位 36 对无感；外推 50 单位 ~3-6ms/tick、100 单位 ~12-25ms/tick）。触发 = 单位规模 50+。设计笔记已记录升级路径。
- **寻路+移动核心 GDExtension 化（用户战略方向，2026-07-02 原话大意）**："如果 C++ 能提升效率，作为最底层的设施（项目层几乎不需要改它，稳定后自身也几乎不用改），把它做好是值得的，可以在各种项目中使用，是非常长期的收益。" 技术注记：① 移植范围应含 long-path core + 分离求解（后者才是 N² 大头）；② **Web/WASM 导出约束**：GDExtension 需另行编译到 wasm（Godot 4.3+ 支持，构建链成本要算进去）；③ C++ 层才能解锁真后台线程规划（GDScript 线程两次验尸判死）。触发 = 1c 落地、移动/寻路 API 与手感契约冻结之后（先稳接口再固化底座）。
