...
### 3b. 录像格式 v3 — split world_snapshot + event_timeline（fable）【🗳️ 提案待批准 2026-07-03】
- **提案（grill 拍板固化）**：[`2026-07-03-playback-v3-format.md`](../../addons/logic-game-framework/docs/proposals/2026-07-03-playback-v3-format.md) —— 7 项拍板（全套 v3 / 删 version / 全体 registry 快照 / 砍 abilities-tags / 旧模式彻底删除 / JS 端欠账 / 大小优化不搭车）+ 反直觉事实链（快照删不掉、文件不会变小、存档序列化不可复用）+ 执行清单。批准后单轮落地。
