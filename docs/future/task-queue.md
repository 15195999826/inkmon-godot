...
### 3d. 刀 9 表演管线上提——hex frontend 表演框架抽进 LGF `presentation/`（fable）【✅ 完成 2026-09-24：PL0–PL5 全部落地；收口 addons `c5e1205` / 主仓 （本行由随后的 docs 提交回填）】
...
- **进度（2026-09-24）**：PL0 钉子 addons `bc385e6` / 主仓 `fac6aaf6` → PL1 坐标对齐 `7c2ae56` / `faf96a47` → PL2 搬家 + 改名 + 扩展缝 `a0c9330` / `97519924` → PL3 Director 骨架 + live 入口 `5a60ae2` / `d61b3af7` → PL4 文档与规则之家（LGF `CLAUDE.md` 分层图 + 三条铁律 +「Presentation layer」节；主仓 `CLAUDE.md` L11；hex `frontend/README.md` 精简；`enforcing-lgf` 指针表加 Presentation 行；逻辑层注释 `BuffVisualizer → BuffTranslator`）SHA 见计划 §6 → PL5 整体审与收口（8 角度审 10 条：修 7、2 条 low 记 §6 后续观察、1 处退役词改名；core 单测 332→333）addons `c5e1205` / 主仓 SHA 见计划 §6。每阶段验收与偏离见计划 §6 / deviations 文件。
...
> **状态**（2026-09-24 刷新）：✅ 完成 = 1a · 1b · 1d · 2a · 线 3 · 3b · 3c · 3d；⏸️ 搁置 = 1c；◐ 半定 = 2b（大地图半边已定，剩战斗地图 Phase 2 定）；📋 待启动 = 2c · 2d · 2e（inkmon 重设计，2c/2d 随之重估）；🔄 进行中 = 无（3d 刀 9 表演管线上提 2026-09-24 收口）。启动某项时，把该项从"登记"推进为"进行中"，产出物（review / 方案 / 提案）另起文档或落到对应区域，本文件只维护队列态。
