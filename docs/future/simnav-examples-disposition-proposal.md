...
`smoke_sim_nav_*`）物理独立于 example，不受影响（执行时修正：015/016 实为 lab 耦合测试、随 lab
连带删除）——本提案开篇「1a 结论 core 是要保住的」的
回归资产依然安全。物理删除 + ~15 处交叉引用清理已于 2026-07-04 执行（submodule 4be5da5 + 主仓
c0cff5e7）；`sim-nav-map-bugfix` probe 工具链重工同日完成（删模板、probe_log.py 对齐 dota2 导出
schema v1），执行记录见 task-queue.md 1b。
