...
- **大地图生成/渲染 v2 ✅ 已落地（2026-07-05，adr/0012）**：技术选型轮（Python mock 四轮对齐方向 → 用户拍板概念图色系 + 追加世界观约束 low poly/faceted/sharp edges/ink sketch/六元素）→ 全链落地：①生成层 = 大陆噪声场（elevation+ridge / moisture / 纬度温度 + domain warp）双域量化（分类=秩归一+hex 低通 / 渲染场=minmax）→ 每格 biome 六种入档 + 河流（最低势能探索 + BFS 河道）入档，`SAVE_VERSION` 4→5，`terrain_at()` 3-kind 投影保住 M2.2 战斗皮肤链路，六元素亲和映射留 biome 挂点待拍板；②渲染层 = shader 地图纸（CPU 只烘 biome/elev/moist 数据纹理 + coast/warp/mottle 辅助场，海岸公式与河流生成同源零漂移），海洋只做视觉边框（可玩矩形恒陆地，节点/选路零逻辑影响），hex 格线与矩形描边删除，迷雾/节点/走廊/旗原样叠加；③风格 = 6 预设纯表现层切换（默认墨线面片；moss/watercolor/plain_wc/flat；**codex 版**=应用户要求由 codex 自主设计的暗色重墨风），HUD 按钮循环 + `user://` 偏好；④shot 逐风格自验多轮迭代（秩场胡椒/warp 灯芯绒/河流电路板蛇形/摩尔纹/岸带矩形环等坑全记代码注释）；用户对照 Python mock 验收指出"3D 起伏/颜色/过渡不如 mock"后**渲染真相改锚**：由"616 格数据纹理放大"改为"连续场纹理逐像素 + 分位 CDF LUT"（mock 管线原样进 shader，分类与逻辑层同阈值同秩域、对齐在格中心），起伏/过渡达 mock 同档；效果对比图留档 `.claude/tmp/map-style-showcase/`；**终局 (A1+A2+A3, 2026-07-05 用户拍板)**：mock_replica 复刻实验 (Godot 内逐像素还原基准图, 均差 1.8%) 证明渲染无损后, 残余差定位为展示几何——①去等轴压扁 (ISO_SQUISH 1.0) ②生成噪声换 mock 值噪声 (uint32 hash 位级确定, SAVE_VERSION 6) ③渲染切 GPU 多 pass (40px/格 mock 同密度, mock 十类分类, 异步烘焙)；游戏内大地图 = 基准图同观感, **用户验收通过 (2026-07-05:“这下看着顺眼太多了, 这个版本就用这个方案”)**。**验收双关卡**：一致性检查（对照 adr/0012 决定一~五逐条，含决定四改锚修订）✅；codex review 一轮 High（河流 coast 噪声未走公共工厂与 shader 漂移）+ Medium（烘焙缓存键缺 cells 本体）+ Low×2（河口↔海岸同源断言、风格切换测试缺口）全部闭环，`smoke_map_style` 新增进 mission 组。
