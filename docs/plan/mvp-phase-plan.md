...
- **M2.2 踩节点开战（logic+host）**：走到 battle 节点 → 战斗地图模板生成（Q2.2 模板制：固定小棋盘 + 按区域地形皮肤 + 随机 0-3 障碍；尺寸授权 fable 定）→ 敌方 = 临时野生 actor（训练假人同款生命周期）→ battle flow 复用（出征中开战沿用训练战的冻结/回放观看语义）。胜 → 回大地图继续；4v4 全自动打到一方全倒，己方全倒 = 全灭 = 既有丢趟出口，天然闭环无新语义。
