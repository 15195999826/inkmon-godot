...
- **已拍板 A 并改写（2026-09-24，后续会话落地，addons `667df6c`）：hex `frontend/README.md`「项目背景」。** 原文称本目录「inkmon 战斗系统的 Godot 3D 表演层」并与 `../inkmon-web/lib/battle-replay/` 对照「1:1 架构」；web 端是否按 adr/0013 词表走未核实、也不在本仓。选项：A 改写成「hex 示例的 3D 表演层 = LGF `presentation/` 的第一消费者」并删 web 对照表 / B 保留，等核实 web 端后再改 / C 删整节（「表演框架在哪」「hex 接线」两节已足够）。→ **落地 A**：改写成两段（hex 示例的 3D 表演层 = LGF `presentation/` 的第一消费者；框架三条约束一句带过），删「设计目标」四条与「相关项目」表，inkmon-web 对照不再提；只动这一节，代码 / inkmon/ / 其他章节不动。
