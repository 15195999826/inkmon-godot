...
内容管线：`tools/ink_mon_content_import_tool.gd` 从 `https://lomo.inkmon.cloud/inkmon/contract`（Basic Auth）fetch → `logic/content/ink_mon_content_importer.gd` + `ink_mon_l2_content_contract.gd::validate_creature_base` 校验 → 写 `res://data/inkmon_content.json` → 启动时 `ink_mon_content_loader.gd::apply_to_runtime` 把每个 unit 的 `base_stats`/`stage` 注册成 `InkMonSpeciesCatalog` override（**只覆盖六维+阶段**；技能池/进化拓扑/阈值仍是 godot stub）。
...
- [ ] **2 · catalog edge-list 真相源**（`ink_mon_species_catalog.gd`）：新增 `static _evolution_edges`/forest（`{parent_species_id → [{child_species_id, trigger}]}`）+ `register_evolution_edges(edges)`（`clear_overrides()` `:85-86` 一并清）；`get_evolution()`（`:127-131`）改查 edge-list、**支持多子**、无 override 时回退 stub；`_build_table()`（`:206-266`）各 `evolves_to` 降级为纯 fallback。`_overrides` 改按 **species_id** 键；`register_override` 形参语义改 species_id；**删 `normalize_species_key()` 双写**（`:85-95,73-76,99-109`）——mon_NNNN 唯一规范键、无大小写歧义。
...
- [ ] **4 · loader**（`ink_mon_content_loader.gd`）：units 循环（`:44-58`）改读 `unit.get("id")` 当 species_id 键、`unit.get("display_name")` 当显示名（**别再读 `unit.species`**）；循环后新增提取 `data.get("evolution_edges", [])` 调 `register_evolution_edges`。
...
- [ ] **8 · 测试 fixture + smoke**：`tests/fixtures/sample_creature_contract.json` 移除 unit 上 `evolves_to`、加根级 `evolution_edges`、`unit.id` 用 `mon_NNNN`；`smoke_content_contract.gd`（断言拒 per-unit evolves_to、接 evolution_edges、id=species_id 形状）、`smoke_content_loader.gd`（override 按 id 命中）、`smoke_progression.gd`（进化走 contract 灌入/stub 边、断言阈值来自 trigger、`entry_id` 不变、X→X2 + 补槽不变）。
