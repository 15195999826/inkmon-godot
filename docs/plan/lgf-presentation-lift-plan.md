...
> 状态：**PL0–PL5 全部完成（2026-09-24）**。各阶段 submodule / 主仓 SHA 见 §6；收口后两仓待用户 push。
...
| PL4 文档与规则之家 | 已完成 2026-09-24 | `9a8ee35` | `75b045ee`（本行由随后的 docs 提交回填） | 验收：`-Required` 21 scene + `hex/all dota2autobattle/smoke inkmon/all core/skill-preview-env` 78 scene 全 PASS（core 单测 332 不变，零代码行为改动）/ golden 三 seed 指纹零漂移（466926191 / 4212318930 / 2532673107）/ leak 直方图仍为空与基线一致 / 完成定义：指针表无死链（`enforcing-lgf`「Where to look」三个 frontend 路径仍在，加一行 Presentation；hex README wire 节 `main.gd` 死指针改 `demo_frontend.gd`）；D8 执行期落地 = LGF `CLAUDE.md`（分层图 Presentation 子图 + 三条铁律 +「Presentation layer」节：词表六件 / `pump` 8 步 / 7 条信号 / 消费方接入清单）· 主仓 `CLAUDE.md` L11 · hex `frontend/README.md` 精简（只剩 wire / demo / 背景 / 表演框架在哪 / hex 接线 / 扩展 / 目录 / 接入清单）· task-queue 3d 推进为「进行中」；PL2 后续观察点名的逻辑层注释 `BuffVisualizer → BuffTranslator` 七处扫完 / inkmon 守卫空。两问自查：本阶段零新增表 / 注册 / 缓存 / 循环（文档 + 注释）。偏离见 deviations 文件 PL4-1–5 |
| PL5 整体审与收口 | 已完成 2026-09-24 | `c5e1205` | `b9c1ae0e`（本行由随后的 docs 提交回填） | 验收：`-Required` 21 scene + `hex/all dota2autobattle/smoke inkmon/all core/skill-preview-env` 78 scene 全 PASS（core 单测 332→333：ReplayDirector +1 无录像无动作 / 零帧录像只报一次；既有用例补幽灵插值、到期标脏、结束后不重报断言）/ golden 三 seed 指纹零漂移（466926191 / 4212318930 / 2532673107，dump 逐字节同 PL4）/ leak 直方图仍为空与基线逐字节一致/ 整体审（PL1–PL4 累计 diff，8 角度 high 档，不含同区间的定向投递刀 2e9a89b）报 10 条：修 7（ReplayDirector `playback_ended` 只报一次 · play / reset / step 无录像无动作 · `load_playback` 走 `reset_to` · `apply_move` 幽灵插值守卫 · `_expire` 一趟分组 + 真归零才标脏 · `pump` 只读视图一趟建一次 · 退役词改名）、2 条 low 不修记后续观察（`seed_actor` hp > max_hp / 账本时钟截断）、1 条随改名关闭/ inkmon 守卫空。两问自查：新增的表 = `ReplayDirector._ended_reported`（`load_playback` / `reset` 归 false，`_exit_tree` 无需）与 `_expire` 趟内局部的 `flashing` / `tinted`；新增循环遍历 `get_procedural_effects()` 账本数组只读无回调，actor 循环仍走 `get_actor_ids()` 快照。偏离见 deviations 文件 PL5-1–8 |

**后续观察**（范围外发现记一行，不顺手修；2026-09-24 分诊、拍板与处置见 [`lgf-presentation-lift-triage.md`](lgf-presentation-lift-triage.md)，每条末尾的 → 标记是处置结果）：

- 候选：effects map 统一（§4 第三条）——四份一次性效果簿记除 dedupe / 过期外无人读取，合成一张表后 view 可按 id 懒建、与 actor 同款 flush。→ **分诊核对已关闭**：PL2-4 已合成 `VisualState._effects: kind → {id → Effect}` 一张表 + `spawn / update / remove / expire_effects` 四原语，view 已可按 id 懒建；无新事可做。
- 候选：`AnimationConfig.from_dict` 无人调用（hex `create_default` 唯一入口），上提后若仍无消费方可删。→ **分诊轮已删**（全仓零调用，只留 `create_default`；addons `878c5b1`）。
- PL0 记：`FrontendStageCueVisualizer.EXECUTE_KILL_VFX_DURATION = 0.8` / `EXECUTE_KILL_VFX_DELAY = 0.15` 疑似按秒写——表演层时间单位是 ms，斩杀特效 0.8 ms 内建了又删（golden 651143 第 18 帧 `attack_vfx delay=0.15 dur=0.80`，同步 +1/-1）。改值会动 golden，等用户拍板后重烤。→ **分诊轮已修**（用户拍板 B）：800 / 150 ms，golden 仅 651143 漂（指纹 466926191 → 2927815786，逐步归因见 deviations TR-3）；addons `878c5b1`。
- PL0 记：三个 golden seed 的 6 张投射物卡 duration 全被 300 ms 下限夹住（demo 地图 hex size 1，世界距离 / 20 单位每秒 ≤ 300 ms）——golden 钉不住 `FrontendProjectileAction.calculate_duration` 的距离公式；PL1 改投射物距离口径（D2）时另用单测钉今日「世界距离 / 速度」的数值，或加一个大地图 seed。→ **PL1 已钉**：`tests/presentation/hex_translator_coordinates_test.gd` 钉投射物距离口径（PL1-7 / PL2-8），关闭。
- PL0 记：`FrontendVisualAction.ActionType.MELEE_STRIKE` 无翻译员产出、RenderWorld 无分支，golden 的 KIND_NAMES 里那一条是死项；PL2 D4 内置 11 种 kind 本就不含它，改名时顺手不带。→ **PL2 已不带**：LGF / hex 零 `MELEE_STRIKE`（仅 inkmon 冻结副本仍有），关闭。
- PL1 记：hex 逻辑层把投射物位置打包成 `Vector3(q, r, 0)`（`HexBattleSkillHelpers.owner_position_resolver`），`ProjectileSystem` 的飞行时间 / `hit_distance` 都按 axial 平面欧氏度量（各向异性：六个邻格里 (1,-1) 比 (1,0) 远 41%）；表演层 PL1 起与它同口径。逻辑层若改成 hex 步数度量，翻译员 `calculate_duration` 那一处同步即可。→ **登记不动**（逻辑层，本轮硬约束；触发 = 逻辑层改 hex 步数度量时同步翻译员一处）。
- PL1 记：`FrontendAnimationConfig.projectile_default_speed` 与 `FrontendProjectileAction.get_trail_length` 无人调用（速度从事件读、拖尾长度 view 自己定），PL2 改名时顺手不带。→ **PL2 已不带**：LGF / hex 零引用（`projectile_default_speed` 仅 inkmon 冻结副本仍有），关闭。
- PL1 记：`FrontendFacingIndicatorView` 仍自己拿 `GridLayout` 算朝向向量（邻格像素差），可改用 `FrontendHexProjection.delta_to_world` 统一投影入口，视觉无差；view 层的事，不急。→ **登记**（view 层候选，见 [`lgf-presentation-lift-triage.md`](lgf-presentation-lift-triage.md) ⚪ 桶）。
- PL2 记：`BuffVisualizer` 一词仍留在逻辑层注释里：`core/events/game_event.gd`、`core/playback/recording_utils.gd`、hex `logic/abilities/active/surge.gd` / `buffs/surge_buff.gd` / `actions/surge_tick_action.gd`、`tests/battle/skill_scenarios/{poison,surge}_scenario.gd`——不在 PL2 清单（其中两个是定向投递刀同日在改的文件），PL4 文档阶段顺手扫成 `BuffTranslator`。→ **PL4 已扫**（PL4-3 七处），关闭。
- PL2 记：`VisualState.set_actor_hp / set_actor_position / set_actor_dead` 三个直接状态更新只有单测在用（production 无消费方）；PL3 照 inkmon 抄 `seed_actor / despawn_actor` 时一并定去留。→ **PL3 已定**（PL3-5：删前两者、`set_actor_position` 归 live 入口），关闭。
- PL2 记：`FrontendBattleDirector` 的组件在 `_ready` 才建（`updater` 也是），animator 只能在 `add_child` 之后 `register_handler`；PL3 抽 `VisualDirector` 时把 registry / stepper / state / updater 挪到 `_init` 建，让「建好 Director 再登记私有卡片」不依赖入树顺序。→ **PL3 已解**（PL3-1 四件 `_init` 建、注册表构造注入），关闭。
- PL3 记：`ReplayDirector.step()` 在 `playback_ended` 之后每调一次都重发 `playback_ended` / `playback_state_changed(false)`（`_process` 路径被 `_is_playing` 挡住，手动步进没挡；沿旧 `_tick`）；golden 停在首个 ended 不受影响。改成只发一次动播放语义，PL5 整体审时定。→ **PL5 已修**：加 `_ended_reported`（`load_playback` / `reset` 归 false），`playback_ended` / 停播只报一次；没有录像时 `play` / `reset` / `step` 无动作。
- PL3 记：`VisualUpdater.apply_move` 先写在飞插值再查 actor 是否在账上，despawn 后 / 未知 actor 的 move 卡会给 `_interpolated_positions` 留一条幽灵项直到下次 `initialize_from_replay`；live 项目 despawn 前 `cancel_for_actor` 即可避开，框架侧候选加 `has_actor` 守卫。→ **PL5 已修**：不在账上的 actor 整张忽略。
- PL3 记：三个白盒 smoke（buff_pipeline / facing_indicator / regeneration）手抄的 `_run_frame` 现在可直接喂 `VisualDirector.pump`（PL2-9 时框架尚无 Director），候选顺手改，本轮不动。→ **分诊轮已改**：`ReplayDirector.load_playback` 重建台面 + 直接 `pump`，账本只走公开读法（deviations TR-1）；addons `878c5b1`。
- PL3 记：Director 四件在 `_init` 建、`_exit_tree` 拆（沿旧 Director）——移出树再加回即不可用；现无消费方这么用，若要支持改成 `NOTIFICATION_PREDELETE` 拆或干脆不拆（Node 目标的信号连接随 free 自动断，无 RefCounted 环）。→ **登记**（无消费方；触发 = 第一个要移出树再加回的消费方，届时改 `NOTIFICATION_PREDELETE` 拆或不拆）。
...
- PL4 记：hex `frontend/README.md`「项目背景」仍称本目录「Godot 3D 表演层」并与 `../inkmon-web/lib/battle-replay/` 对照「1:1 架构」——web 端是否也按 adr/0013 词表走未核实，等用户拍板后再改。→ **已拍板 A 并改写**（addons `667df6c`；拍板记录见[分诊文档](lgf-presentation-lift-triage.md) §2）。
