...
| PL5 整体审与收口 | 已完成 2026-09-24 | `c5e1205` | （本行由随后的 docs 提交回填） | 验收：`-Required` 21 scene + `hex/all dota2autobattle/smoke inkmon/all core/skill-preview-env` 78 scene 全 PASS（core 单测 332→333：ReplayDirector +1 无录像无动作 / 零帧录像只报一次；既有用例补幽灵插值、到期标脏、结束后不重报断言）/ golden 三 seed 指纹零漂移（466926191 / 4212318930 / 2532673107，dump 逐字节同 PL4）/ leak 直方图仍为空与基线逐字节一致/ 整体审（PL1–PL4 累计 diff，8 角度 high 档，不含同区间的定向投递刀 2e9a89b）报 10 条：修 7（ReplayDirector `playback_ended` 只报一次 · play / reset / step 无录像无动作 · `load_playback` 走 `reset_to` · `apply_move` 幽灵插值守卫 · `_expire` 一趟分组 + 真归零才标脏 · `pump` 只读视图一趟建一次 · 退役词改名）、2 条 low 不修记后续观察（`seed_actor` hp > max_hp / 账本时钟截断）、1 条随改名关闭/ inkmon 守卫空。两问自查：新增的表 = `ReplayDirector._ended_reported`（`load_playback` / `reset` 归 false，`_exit_tree` 无需）与 `_expire` 趟内局部的 `flashing` / `tinted`；新增循环遍历 `get_procedural_effects()` 账本数组只读无回调，actor 循环仍走 `get_actor_ids()` 快照。偏离见 deviations 文件 PL5-1–8 |
