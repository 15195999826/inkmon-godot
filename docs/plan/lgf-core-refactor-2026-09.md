...
**文档**：CHANGELOG Changed（grid 成 stdlib 电池、core 只留 `_get_map_config()`、dota2 不再白带）；`docs/README.md` 的「WorldGameplayInstance 把 grid 概念塞进 core」条 → 已知债务 ✅，目录树 `├── stdlib/` 块加 `stdlib/grid/`，「新模型确立」段里 world 持有 `grid` / `configure_grid` 的描述改指 `GridWorldGameplayInstance`；LGF `AGENTS.md`/`CLAUDE.md` mermaid 加 `Grid[GridWorldGameplayInstance]`；`docs/main-game-architecture.md` `:86`（`overworld_grid` 必留 → 分工固定，`get_battle_grid()` 是唯一战斗侧读名）、`:117` ⚠️ → 已解决、`:263/:265` §9；skill `reference/entity.md`（WorldGameplayInstance 去 grid，新增 GridWorldGameplayInstance 节）。
