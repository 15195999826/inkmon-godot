...
- **`HexBattleProcedure` 照样 tick 死者的 ability**（P5 发现，既有）：`tick_once` 里补 tick 中途 spawn 的那段循环按「不在存活 character 名单里」挑 actor，死掉的参战 character 也被挑中，尸体上的 DOT（毒等周期 timeline）继续结算伤害、推 damage 事件；P5 之后尸体响应自己挨的 damage，这类伤害还会触发尸体的荆棘（见 ㊻）。修向：该循环跳过 `is_dead()` 的 character，或 DOT action 对死亡 owner 早返回——先确认尸体上的 buff 是否该继续倒计时（`TimeDurationComponent` 也走这条 tick）。（P10 整体审补充：同一循环 `for actor in world.get_actors()` 遍历的是 registry 活数组，循环内 totem / fire tile 寿命到期 `remove_actor` 会让下一个 actor 本帧跳过一次 `tick_runtime`；死者未完成的 Move execution 仍 fire `ApplyMoveAction`，对已清占用的起点 `move_occupant` 失败并顺手取消目标格他人预订 + UNEXPECTED push_error——`GridMapModel.move_occupant` 先 `cancel_reservation(to)` 再判失败是 ultra-grid-map 既有行为。均早于 8e56bba。）（2026-09-14：补 tick 循环已跳过 `is_dead()`，尸体不再 tick——DOT / 荆棘 / 死后余波这一半关闭，见 deviations RD-3；遍历中 `remove_actor` 跳 tick 一条仍开；死者的 Move execution 随尸体冻结不再 fire，但 execution 实例未 cancel。）
- **周期 buff 在被 grant 的那一帧不再 tick**（2026-09-14 随机战斗差分发现，P5 `_process_abilities` 遍历快照的副作用，行为变化未登记）：基线把 grant 帧重复计入，第二次 tick 落在第 19 帧；HEAD 落在精确周期第 20 帧。含周期 buff 的单位 ATB 时机最多偏 1 tick，数值与胜负不变。建议保留 HEAD，`smoke_random_battle_golden` 钉住；若要回到基线行为，需让快照遍历补处理本趟新 grant 的 ability，并接受「同帧是否 tick」取决于目标处理顺序。
