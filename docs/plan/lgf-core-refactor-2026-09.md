...
- **`AbilitySet.revoke_ability` 先定位下标、再 `expire()`、最后 `remove_at(index)`**（2026-09-17 B-1 发现，既有）：ability 的 on_remove 里若 revoke 了排在自己**前面**的兄弟，下标已左移，`remove_at` 删错人或越界（自己以 EXPIRED 态残留到下一次 `_process_abilities` 清扫）。今天无生产 on_remove 会 revoke 兄弟（hex 护盾破裂由伤害管线从外部 revoke），`ability_set_revoke_test` 的级联用例特意让级联目标排在级联者之后、不依赖这条。修向：expire 之后按对象 `find` 再删（或 `erase(ability)`），补一条「级联目标在前」用例。
