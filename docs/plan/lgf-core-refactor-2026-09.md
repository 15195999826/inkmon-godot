...
> 状态：**P1–P10 全部完成（2026-09-12）**，本轮收口；**2026-09-12 流程修订**（D10 / D11 / §2 / §5 重写，新增 P5.5 文档清场与 P10 整体审）。四条设计题（见 P10 改法 5）留用户拍板。每阶段完成后在 §6 填 submodule / 主仓 SHA。
...
5. **留给用户拍板、本阶段不做**：注册前 grant 的纯 post 被动静默缺订阅；post handler 里过期不当场 revoke；`Ability.owner_actor_id` 与 set owner 双来源；Break 后 pre handler 仍改事件。整体审若再报这四条，标「已知、待拍板」不修。
...
- **`BattleRecorder.unregister_actor()` 全仓零调用点**（P2 发现）：`WorldGameplayInstance` 有 `actor_removed` signal，`BattleProcedure` 只接了 `actor_added` 补录那一半。hex 死者留 world 不 `remove_actor`、dota2 已关录像，所以当前无现症；但只要有一家开着录像又移除 actor，订阅就会留到 `stop_recording`。接线时注意 `record_actor_lifecycle` 的 despawn listener 也会推 `ActorDestroyed`，别和 unregister 各推一条。（P10 整体审复核：hex totem / fire tile 寿命到期经 `instance.remove_actor` 出 registry 即现症——`remove_actor` 只注销事件 handler，`BattleRecorder.actor_subscriptions` 的退订闭包仍强持该 actor 连同 ability / execution 到 `stop_recording` / `abort_recording`；有界（战斗内）、不改结果，low。修向：`BattleProcedure` 接 `actor_removed` → `recorder.unregister_actor`。）
...
- **hex 伤害流程参考文档仍写「死后 remove_actor」**（P3 复审第四轮发现，既有，2026-04-26「死者留 world」起过期）：`example/hex-atb-battle/docs/reference/damage-pipeline.md` 流程图 ⑧ 与「关键顺序约束」表两行（「on_break 在 remove_actor 之前，否则拿不到 owner 上下文」「post_event 在 remove_actor 之后，需要尸体已不在战场」）、`shield-system.md:43/48/185`。代码注释已在 ㉔ 改正；这两份要按「死亡检测锁存 `is_dead()` 并清 grid 占用，位置与 ability 仍在」重写顺序约束的**理由**，不是换词。`smoke_world_view.gd` 空 view 分支的通过文案（「战斗期 remove signal 已验证」）同样过期。（P5.5 两份文档已删，顺序约束的理由已按「死亡锁存 `is_dead()` + 清 grid 占用、不 remove_actor」改写进 hex `README.md` 设计铁律；`smoke_world_view.gd` 文案仍开。）
- **`GameWorld.destroy_all_instances()` 与 `shutdown()` 行为已完全相同**（P4 发现）：两者都是「结束全部 instance + 清注册表」，只差日志级别（debug / info）。计划的保留清单两个都列了；inkmon host 的 `reset_session` / `load_game` 与 5 个 inkmon smoke 用前者，场景收尾用后者。要么删 `destroy_all_instances` 统一成一个动词，要么给两者定下可区分的语义。
- **`GameWorld.create_instance` 撞 id 时返回旧实例**（P4 发现，既有语义）：警告后返回已注册的那个；改成 instance 形态后调用方多半丢弃返回值、继续用手里那个没注册进去的新实例，而 context 反查到的是旧实例——静默错位。P4 未改语义。修向：同 id 异对象 `Log.assert_crash`，同一对象重复注册保持幂等。
...
- **`finish()` 清 in_combat 产生的 `TagChanged` 漏进下一场录像首帧**（P4 复审第 2 轮发现，P4 之前即如此）：`BattleProcedure.finish()` 在本场最后一次 `record_current_frame_events()` 之后、`stop_recording()` 之前 `_mark_in_combat(pid, false)`，录像订阅仍开着，事件进了 world 的 `event_collector`；`stop_recording` 不 flush，`InkMonWorldGI._reset_battle_state` 与 `SkillPreviewWorldGI.reset()` 也不清，下一场第一次 `tick_once` 的 flush 把它们写进第 1 帧——常驻世界里一场录像的内容取决于之前打过几场。更一般地，world 的 collector 只在战斗 tick 里 flush，两场之间推进去的事件都会攒进下一场首帧。今天读 `tagChanged` 的只有 `PlaybackLogPrinter` 与 hex `smoke_random_frontend_20_runs`，玩法不受影响。修向：`finish()` 在 `stop_recording` 前把 collector 余量录进最后一帧，`_start_recorder()` 开录前丢弃 collector 里的旧事件。
- **战斗 tick 内结束 world 时，正在执行的 `tick_once()` 仍跑完函数体**（P4 复审第 2 轮发现）：`WorldGameplayInstance.tick()` 只在 `tick_once()` 返回后才发现槽位已交还；hex / inkmon / skill-preview 的 `tick_once` 只在入口判一次 `_finished`，余下的 actor 循环照常执行，此时注册表已注销该 world、`ctx.instance` 反查为 null，走 `HexBattleGameStateUtils.world(ctx)` 的 hex action 会逐个断言。生产代码没有在战斗 tick 里结束 world 的路径（inkmon `smoke_world_command` 是先 `tick_once` 再在外层 `destroy_all_instances`）。修向：procedure 的 actor 循环里每个 actor 前再判 `_finished`。
- **`HexBattleProcedure` 照样 tick 死者的 ability**（P5 发现，既有）：`tick_once` 里补 tick 中途 spawn 的那段循环按「不在存活 character 名单里」挑 actor，死掉的参战 character 也被挑中，尸体上的 DOT（毒等周期 timeline）继续结算伤害、推 damage 事件；P5 之后尸体响应自己挨的 damage，这类伤害还会触发尸体的荆棘（见 ㊻）。修向：该循环跳过 `is_dead()` 的 character，或 DOT action 对死亡 owner 早返回——先确认尸体上的 buff 是否该继续倒计时（`TimeDurationComponent` 也走这条 tick）。（P10 整体审补充：同一循环 `for actor in world.get_actors()` 遍历的是 registry 活数组，循环内 totem / fire tile 寿命到期 `remove_actor` 会让下一个 actor 本帧跳过一次 `tick_runtime`；死者未完成的 Move execution 仍 fire `ApplyMoveAction`，对已清占用的起点 `move_occupant` 失败并顺手取消目标格他人预订 + UNEXPECTED push_error——`GridMapModel.move_occupant` 先 `cancel_reservation(to)` 再判失败是 ultra-grid-map 既有行为。均早于 8e56bba。）
- **同一 ability 挂两个同 kind 的 `PreEventComponent` 时注册 id 相同**（P5 发现，既有）：id 是 `"<ability_id>_pre_<kind>"`，按 id 注销会删掉先匹配到的那一条（可能是另一个 component 的）。post 按 kind 去重、每种 kind 一条，没有这个问题。
- **core 单测日志的刻意报错基线**（P5）：扫日志时「EventProcessor not available」警告一条（`instance_context_test`）、「Event recursion depth exceeded」错误一条（`post_event_dispatch_test` 的嵌套派发用例），多出来的才是问题。
- **注册进 GameWorld 之前 grant 的纯 post 被动永久缺订阅**（P5 审查发现，计划如此）：`Ability._register_post_handlers` 在 context 没有 processor 时静默返回；P5 之前广播在派发时按 id 反查，注册前 grant 无害。「EventProcessor not available」探针只覆盖带 `PreEventComponent` 的 ability，纯 NoInstance / ActivateInstance 的被动（荆棘、亡语）没有信号。仓内生产路径都先注册再 grant。修向：processor 缺席时与 `PreEventComponent.on_apply` 同样打警告，或 `grant_ability` 断言 owner 已注册（孤立单测要一起改）。
- **post handler 里过期的 ability 不被当场 revoke**（P5 审查发现）：post 派发直接调 `Ability.receive_event`，不经 `AbilitySet._process_abilities` 的过期清扫；一次性被动在自己触发的 action 里 `expire()` 之后，以 EXPIRED 状态留在 set 里，直到下一次经过这个 AbilitySet 的 tick 或定向投递（grant、激活请求）才被清扫，期间 `has_ability` 仍命中、`abilityRemoved` 延后。hex 只在护盾破裂一处当场 revoke。修向：`Ability._make_post_handler` 在 `receive_event` 之后发现过期即 `context.ability_set.revoke_ability(...)`，或让 `expire` 通知所属 set。
- **core 没有「整个换掉 AbilitySet」的出口**（P5 审查发现）：inkmon 为同一根因手写两处清理——`InkMonUnitActor.reset_battle_runtime` 按 owner 注销 handler、`InkMonUnitActor._clear_equipment_abilities` 按 source 清装备 modifier，其余 `on_apply` 留下的外部状态换集时照样残留。修向：`AbilitySet.revoke_all(reason)`，换集前调它。
- **投射物 HIT / MISS 的 post 派发仍由 example 层回扫 collector**（P5 审查发现）：hex procedure、skill-preview procedure 与测试 harness 各自手动调 `HexWorldGameplayInstance.broadcast_projectile_events`；观众参数删掉之后，stdlib `ProjectileSystem` 可以在产生事件处直接派发。
- **`Ability.owner_actor_id` 与所在 AbilitySet 的 owner 是两个来源**（P5 复审发现，既有）：pre / post 注册取 set 的 owner（lifecycle context），`AbilityLifecycleContext.for_ability` 与 `AbilityRef.from_ability` 取 `Ability.owner_actor_id`。构造时传的 owner 与被 grant 进的 set 不一致时，注册与派发正常，on_remove / 叠层 / Break 钩子与 execution 却反查不到 instance（`TagComponent` 的 tag 撤不掉、timeline action 拿到 null instance）。生产里 `Ability.new` 都传被 grant 的 actor 的 id。修向：`AbilitySet.grant_ability` 断言两者相等，或把 ability 的 owner 同步成 set 的 owner。
- **处置表之外仍指向已删文档 / 已删目录的文件**（P5.5 发现，按「不在表内一律不动」未动）：根 `AGENTS.md`（`.agents/skills/` 与 `/update-lgf-skill`）、dota2 `README.md`（`docs/README.md` / `action-architecture.md`）与 dota2 `CHANGELOG.md`（同样该随「历史归 git log」退役）、`docs/future/task-queue.md`（proposals / `docs/README.md`）、`docs/adr/0005`（proposals，历史记录可留）、`addons/lomolib/docs/dev-agent-debug-mode-spec.md`（`.agents/skills`）。
- **action / condition / cost 的 `TYPE` 类型 id 拼写混杂**（P7 发现，未动）：core 自带的是 snake（`"base"` / `"noop"` / `"skill_local"`），其余是 camel（`"launchProjectile"` / `"stageCue"` / `"looseTagApply"` / `"looseTagRemove"` / `"hasTag"` / `"noTag"` / `"tagStacks"` / `"consumeTag"` / `"removeTag"` / `"addTag"`）。不是事件 key / kind，全仓无人按字面量匹配，只进 `serialize()` 的 `type` 值；要统一另开 chore。
- **`maxHp` 属性名兜底只剩夹具在用**（P7 发现）：hex / inkmon 两份 `render_world.gd`、`skill_scenario_harness`、`smoke_skill_preview_environment` 都写 `attributes.get("maxHp", attributes.get("max_hp", …))`，而所有 attribute config 定义的都是 `max_hp`，`maxHp` 只出现在 hex 前端 smoke 手写的 `actor_init.attributes` 夹具里。可删兜底、夹具改 `max_hp`。
- **`AbilityGranted` payload 里 `id` 与 `instance_id` 双写**（P7 发现）：`battle_recorder._record_existing_actor_abilities` 与 `recording_utils` 的 granted 回调各在 `ability.serialize()` 上再塞一份 `instance_id`，消费方（hex buff / shield visualizer、`smoke_random_frontend_20_runs`）读 `instance_id` 再兜底 `id`。可收成一个键。
...
- **`smoke_summon_spike.tscn` 不在任何 test group**（P9 发现）：四个阶段各自 `instance.grid = UGridMap.model` 直赋绕过 `configure_grid`（不发 `grid_configured`），且从不随 launcher 跑。要么登记进 hex 组并改走 `configure_grid`，要么删。
- **stdlib grid 单测未钉两条 D8 合同**（P10 整体审，low）：`tests/stdlib/grid/grid_world_gameplay_instance_test.gd` 里没有 `hex_position` 的 `plain` 不持预订，「预订按 id 无条件扫（坐标无效也扫）」与「`remove_actor` 先清足迹再出 registry（`actor_removed` handler 内读棋盘已是清后状态）」两条只存在于实现，改回旧形状测试仍绿。
...
- **`register_dynamic_dep` 不发 `_notify_changes`**（P10 整体审范围外，既有）：Vigor / Vitality grant 改变 max_hp / atk（以及由此封顶的 hp 读值）不出 `AttributeChanged`；资源上限经 `update_modifier` / `register_dynamic_dep` 变化的封顶路径也无单测（P10 只补了 `add_modifier` / `remove_modifiers_by_source` 入口）。
