...
- **`u_grid_map.gd` 成了无人注册的可选脚本**（2026-09-17 B-7 发现）：插件不再自动注册 autoload、仓内零引用，只剩 ultra-grid-map README 的示例仍按 `UGridMap` 写；要么删脚本连同 README 段，要么留给其他消费者手动注册。B-7 按拍板只去掉自动注册，未删。
- **post 派发可以在 `base_tick` 遍历 `_systems` 中途跑 handler**（2026-09-17 B-2 发现，既有形状放大）：`ProjectileSystem` 在 system tick 里当场 `process_post_event`，handler 若 `add_system`（就地 sort `_systems`）会让 `base_tick` 的 `for system in _systems` 漏 / 重 tick 某个 system；今日无 handler 加 system。修向：`base_tick` 遍历快照，或 `add_system` 延迟到 tick 外。
