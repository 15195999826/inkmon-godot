...
**4.2b IWorldQuery** — Logic 暴露给 Presentation 的**只读 snapshot + submit facade 对象**(`RefCounted`,私有包 `InkMonWorldGI`;读表面 = `get_player_hud_summary`/`get_roster_snapshot`/`get_bag_snapshot`(含 item config 展示字段,UI 不直触 ItemSystem)/`get_npc_defs_snapshot`/`near_npc_id`/`get_player_coord`/`get_player_actor_id`/`is_player_moving`/`get_npc_actions`/`has_npc_handler`,全部**值拷贝**,不外递活 actor / 内部 Dict 引用 + `submit(cmd)` 写)。结构仿 LGF `BaseGeneratedAttributeSet`(持底层对象 + 受控表面),但**无 `get_gi()` 逃逸口** → Presentation 物理上够不到 concrete GI / flow / lifecycle,也够不到可变域对象(**结构隔离**;smoke 白名单断言钉死公开表面,新逃逸口默认 fail)。GDScript 无 interface 关键字 + GI 单继承位被 `WorldGameplayInstance` 占,故用此 Facade 实现"持接口不持实现";mutation signal 由 Host 连(表演不持 gi)。
