...
##   (没真数据时凑数, 不手敲)。本地静态 content 命中时改用该物种自己的显式 base,
##   取代上面的 stub 占位 (见 _static_species)。最终战斗属性 = base × 等级缩放 (见
...
##   阈值 trigger.level 是设计数据, 住本地静态 content; 评估住 godot
...
##
## stub fallback (Non-Goal): 下面 _build_table 物种 / 池 / 链全为手搓占位, 仅"无静态 content"时生效。
...
## Local static content loaded from res://data/inkmon_content.json on first catalog read.
## Keyed by species_id (mon_NNNN) → {base_stats, stage, elements, display_name}.
## The file is generated by the editor import tool; runtime does not fetch server data
## and does not expose a dynamic override API.
static var _static_content_loaded := false
static var _static_content_source := "unloaded"
static var _static_species: Dictionary = {}

## Local static evolution topology (adr/0010), keyed by parent_species_id →
## Array[{child_species_id, trigger:{level, condition}}]. Authority is PER-SPECIES:
## a species_id present here uses these edges; absent species falls back to stub
## `evolves_to`, so partial content does not strand stub/adopted species.
static var _static_evolution_edges: Dictionary = {}


static func reload_static_content_for_tests(path: String = InkMonContentLoader.DEFAULT_PATH) -> Dictionary:
	var result := InkMonContentLoader.load_static_content(path)
	_install_static_content(result)
	return result


static func replace_static_content_for_tests(species_table: Dictionary, evolution_edges: Dictionary) -> void:
	_static_content_loaded = true
	_static_content_source = "test"
	_static_species = species_table.duplicate(true)
	_static_evolution_edges = evolution_edges.duplicate(true)


static func clear_static_content_cache() -> void:
	_static_content_loaded = false
	_static_content_source = "unloaded"
	_static_species.clear()
	_static_evolution_edges.clear()


static func clear_static_content_cache_for_tests() -> void:
	clear_static_content_cache()


static func has_species(species_id: String) -> bool:
	if not _static_content_for(species_id).is_empty():
		return true
	return _species_table().has(species_id)


static func get_stage(species_id: String) -> String:
	var content := _static_content_for(species_id)
	if not content.is_empty():
		return str(content.get("stage", STAGE_BABY))
	return str(_species_node(species_id).get("stage", STAGE_BABY))


## 物种 base 六维 (level-1 baseline)。命中本地静态 content → 直接返回该物种自己的显式 base
...
static func get_base_stats(species_id: String) -> Dictionary:
	var content := _static_content_for(species_id)
	if not content.is_empty():
		return (content.get("base_stats", {}) as Dictionary).duplicate(true)
...
## 物种元素。命中本地静态 content → 读取 content 的 elements;否则委托 battle 层 UnitConfig
...
static func get_elements(species_id: String) -> Array[String]:
	...
	if not content.is_empty():
		...
		for element_value in (content.get("elements", []) as Array):
			result.append(str(element_value))
		return result
	return InkMonUnitConfig.get_elements_for_species(species_id)


## 物种显示名 (name_en)。命中本地静态 content → 读取 display_name; 否则回退 stub
...
static func get_display_name(species_id: String) -> String:
	var content := _static_content_for(species_id)
	if not content.is_empty():
		var content_name := str(content.get("display_name", ""))
		if content_name != "":
			return content_name
...
static func _ensure_static_content_loaded() -> void:
	if _static_content_loaded:
		return
	_install_static_content(InkMonContentLoader.load_static_content())


static func _install_static_content(result: Dictionary) -> void:
	_static_content_loaded = true
	_static_content_source = str(result.get("source", "stub"))

	var species_value: Variant = result.get("species_table", {})
	var species_table: Dictionary = species_value if species_value is Dictionary else {}
	_static_species = species_table.duplicate(true)

	var edges_value: Variant = result.get("evolution_edges", {})
	var evolution_edges: Dictionary = edges_value if edges_value is Dictionary else {}
	_static_evolution_edges = evolution_edges.duplicate(true)


static func _static_content_for(species_id: String) -> Dictionary:
	_ensure_static_content_loaded()
	if _static_species.is_empty():
		return {}
	return _static_species.get(species_id, {})


## 技能池 / 进化只在 stub 表里。content-only 物种(creature 基底, 无技能/进化元数据,
## skill 是后续阶段 Non-Goal) → 优雅返回 0 槽 / 空池 / 无进化, 而非 assert。用 _node_or_empty
## 而非 _species_node: content-only 给 {}, 真未知才 assert。
...
## 无出边 / 孤儿 / content-only 物种 → []。支持多子分支 (同 parent 多条边)。
## 权威性是 **per-species**: 该 species_id 在静态 content
## 边表中 → 用边表 (权威, 不碰 stub); 不在 → 降级 _build_table 的 evolves_to 单边 (无 condition)
## 作 fallback。⚠ 不能用 `_static_evolution_edges.is_empty()` 做全局门: 那样一旦加载任一 content 边,
## 所有未列入边表的 stub/领养物种 (cinder_kit 等) 就再也查不到自己的 stub 进化链 (混合/过渡态 bug)。
## content 孤儿 (有静态 species 无边) → 不在边表 → 走 stub fallback → _node_or_empty 给 {} → [] (正确无进化)。
static func get_evolution_edges(species_id: String) -> Array[Dictionary]:
	_ensure_static_content_loaded()
	if _static_evolution_edges.has(species_id):
		var out: Array[Dictionary] = []
		for edge_value in (_static_evolution_edges.get(species_id, []) as Array):
...
## stub 表节点, 或 {} 表示 content-only 物种(有静态 content 但不在 stub 表 → 无技能池/无 stub 进化:
## content 只搬 creature 基底, 技能元数据是后续阶段)。仅"既非 stub 又无静态 content"的真未知物种
...
static func _node_or_empty(species: String) -> Dictionary:
	...
	if table.has(species):
		return table[species]
	if not _static_content_for(species).is_empty():
		return {}
