...
`run_tests_headless.gd` / `run_tests_cli.gd` 硬编码测试脚本列表（指向 `addons/logic-game-framework/tests/`），新增 LGF 单元测试需在这里登记。

### 冒烟测试（端到端回归）

走独立 `.tscn` 入口，不登记到上面的 runner（需要 SceneTree `_process` 驱动，模型不兼容）。

| 测试 | 覆盖 |
|---|---|
| `tests/smoke_strike.tscn` | Strike 伤害 resolver 契约（读 caster.atk） |
| `tests/smoke_frontend_main.tscn` | **frontend 完整链路**：main.tscn → start battle → core 跑完 → director 播放到 scheduler 排空 |

```bash
godot --headless --path . res://tests/smoke_frontend_main.tscn
# 退出码 0 = PASS；输出行 "SMOKE_TEST_RESULT: PASS|FAIL - <reason>"
```

`smoke_frontend_main` 设计为**结构不变量断言**（frame 推进、unit 数量 > 0、hp ∈ [0, maxHp]），**不断言具体数值/事件内容**，因此新加技能/单位/Visualizer 通常不需要改测试文件——只要 main.tscn 能跑通就继续守门。加新功能后建议顺手跑一遍。

### 既知噪声

Godot 退出时 `60 resources still in use` / `ObjectDB instances leaked` 警告是 main.tscn 流程的既有资源泄漏（多半是 RefCounted 循环引用或未断开的信号），**不影响退出码，不由本次改动引入**。修要去 LGF addon 内部排。
