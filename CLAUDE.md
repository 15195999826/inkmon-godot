...
目前有两个示例：[hex-atb-battle](addons/logic-game-framework/example/hex-atb-battle/) (回合制 + hex grid) 与 [rts-auto-battle](addons/logic-game-framework/example/rts-auto-battle/) (实时连续坐标 + 自研 grid + A* 寻路 + Activity / AutoTarget / Production / 玩家命令 / 飞行单位 / 城堡战争最小可玩 demo, M1 重构完成)。
