...
- **傀儡测试**：RefCounted 升类的合法理由 = ① 自己要记 transient 私有 state ② 多态派发（无状态 handler/strategy，gi 走参数不走字段）；两者皆无 → 退回 static 纯函数
...
`Log` / `IdGenerator` / `GameWorld` / `WaitGroupManager` / `ItemSystem`
...
约定全文已迁到 project skill [`ui-interaction-smoke`](.claude/skills/ui-interaction-smoke/SKILL.md)——写/改 UI 交互 smoke（`Viewport.push_input` 真注入鼠标键盘）时按需加载，不再常驻。

## Web / JS 桥接

细节已迁到 [`scripts/CLAUDE.md`](scripts/CLAUDE.md)（动 `scripts/` 下 web 桥代码时自动加载）。入口 `SimulationManager._setup_js_bridge()`，只在 `OS.has_feature("web")` 下注册。
...
- 改代码前先确认 scope（改 `inkmon/` 主游戏 / `scripts/` web 桥 / `addons/` submodule），不擅自扩大范围
